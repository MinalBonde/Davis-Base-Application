...
public class Table{
	...
	public static String catalog_dir = "data/catalog/";
	public static String userdata_dir = "data/user_data/";
	...
	public static int countPages(RandomAccessFile file){
		int pageCount = 0;
		try{
			pageCount = (int)(file.length()/(new Long(pageSize)));
		}catch(Exception e){
	...
	public static void drop(String table){
		try{
			...
			RandomAccessFile file = new RandomAccessFile(catalog_dir+"davisbase_tables.tbl", "rw");
			int numOfPages = countPages(file);
			...
			file = new RandomAccessFile(catalog_dir+"davisbase_columns.tbl", "rw");
			numOfPages = countPages(file);
			for(int page = 1; page <= numOfPages; page ++){
				file.seek((page-1)*pageSize);
				byte fileType = file.readByte();
				if(fileType == 0x0D)
			...
			File anOldFile = new File(userdata_dir, table+".tbl"); 
			anOldFile.delete();
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public static String[] getValues(RandomAccessFile file, long loc){
	...
	public static void createTable(String table, String[] col){
		try{	
			...
			RandomAccessFile file = new RandomAccessFile(userdata_dir+table+".tbl", "rw");
			file.setLength(pageSize);
			file.seek(0);
			file.writeByte(0x0D);
			file.close();
			
			file = new RandomAccessFile(catalog_dir+"davisbase_tables.tbl", "rw");
			...
			insertInto("davisbase_tables", values,catalog_dir);
			...
			for(int i = 0; i < col.length; i++){
				...
				insertInto("davisbase_columns", value,catalog_dir);
			}
	...
	public static String[] getDataType(String table){
		...
		try{
			RandomAccessFile file = new RandomAccessFile(catalog_dir+"davisbase_columns.tbl", "rw");
			Records records = new Records();
				...
				byte pageType = file.readByte();
					...
					byte numOfCells = BPlusTree.getCellNumber(file, page);
					 ...
					 for(int i=0; i < numOfCells; i++){
						...
						boolean check = comparatorCheck(vals, rowid , cmp, columnName);
	...
	public static boolean comparatorCheck(String[] values, int rowid, String[] cmp, String[] columnName){
